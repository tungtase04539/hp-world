...
const q = [cell(500, -1300)]; // giữa sông Cấm (1:1)
reach[q[0]] = 1;
while (q.length) {
  const id = q.pop();
...
function checkNear(name, x, z) {
  ...
  for (let dx = -500; dx <= 500; dx += STEP) for (let dz = -500; dz <= 500; dz += STEP) {
...
checkNear('Cửa sông Cấm', 7600, 100);
checkNear('Bến Nghiêng Đồ Sơn', 11700, 17300);
checkNear('Hòn Dấu', 13990, 21100);
checkNear('Bến Cát Bà', 37900, 14900);
checkNear('Vịnh Lan Hạ', 39600, 15400);
